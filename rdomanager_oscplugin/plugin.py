...
class ClientWrapper(object):
    ...
    def baremetal(self):
        ...
        if self._baremetal is None:
            ...
            endpoint = self.instace.get_endpoint_for_service_type(
                "baremetal",
                region_name=self.instace._region_name,
            )

            token = self.instace.auth.get_token(self.instace.session)

            baremetal_client = ironic_client.get_client(
                os_auth_token=token,
                ironic_url=endpoint)
