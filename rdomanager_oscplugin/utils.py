...
def make_nodes_available(self, baremetal_client):
    ...
    for node in baremetal_client.node.list():
        ...
        baremetal_client.node.set_provision_state(node.uuid, 'provide')

        if not wait_for_provision_state(
                baremetal_client, node.uuid, 'available'):
            print("FAIL: State not updated for Node {0}".format(
                  node.uuid, file=sys.stderr))


def get_heira_password(password_name):
