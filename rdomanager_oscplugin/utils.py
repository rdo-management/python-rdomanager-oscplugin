...
def set_nodes_state(baremetal_client, nodes, target_state, result,
                    skipped_states=()):
    ...
    :param target_state: The state to set for a node. The full list of states
                         can be found in ironic.common.states.
    :type  target_state: ironicclient.v1.client.Client

    :param skipped_states: Instance of Ironic client
    :type  skipped_states: ironicclient.v1.client.Client
    """
    ...
    for node in nodes:
        ...
        log.debug(
            ...
            .format(node.provision_state, target_state, node.uuid))

        baremetal_client.node.set_provision_state(node.uuid, target_state)

        if not wait_for_provision_state(baremetal_client, node.uuid, result):
