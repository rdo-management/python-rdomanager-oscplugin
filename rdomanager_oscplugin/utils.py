...
def register_endpoint(name,
    ...
    service = SUFFIXES.get(endpoint_type, None)

    if not service:
        # TODO(bcrochet): Better exception
        raise Exception

    suffix = service['suffix']
    admin_suffix = SUFFIXES.get('admin_suffix', suffix)
    ...
    admin_role_id = (role.id for role in roles if role.name in 'admin')
    ...
    if endpoint_type not in 'dashboard':
        ...
        service_project_id = (project.id for project in
        ...
        user_id = (user.id for user in users if user.name in name)
        if not user_id:
        ...
        if endpoint_type in 'metering':
            admin_project_id = (project.id for project in
            ...
            if not role:
                ...
                # Ceilometer user
                reseller_admin_role_id = (role.id for role in roles if
