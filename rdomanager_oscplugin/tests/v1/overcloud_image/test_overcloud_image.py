...
from rdomanager_oscplugin.tests.utils import mock_open
...
class TestOvercloudImageBuild(TestPluginV1):
    ...
    def test_overcloud_image_build_fedora_user_no_cache(
        ...
        requests_get_response.content = "FEDORAIMAGE"
        mock_requests_get.return_value = requests_get_response

        parsed_args = self.check_parser(self.cmd, arglist, verifylist)
        mock_open_context = mock_open(data=StringIO(
            "Red Hat Enterprise Linux"))

        with mock.patch('__builtin__.open', mock_open_context, create=True):
            self.cmd.take_action(parsed_args)
    ...
    def test_overcloud_image_build_overcloud_full(
        ...
        self.cmd._disk_image_create.assert_called_once_with("-a amd64 -o "
            ...
            "element-manifest network-gateway epel rdo-juno rdo-release "
            "tripleo-image-elements undercloud-package-install "
