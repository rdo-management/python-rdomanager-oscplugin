...
class DeployOvercloud(command.Command):
    ...
    def _create_overcloudrc(self, stack, parsed_args):
        ...
        rc_params.update({
            'OS_PASSWORD': subprocess.call(
                "os-apply-config -m %(nodes_json)s --type raw "
                "--key overcloud.password" %
                {'nodes_json': parsed_args.nodes_json}),
            'OS_AUTH_URL': subprocess.call(
                "os-apply-config -m %(nodes_json)s --type raw "
                "--key overcloud.endpoint" %
                {'nodes_json': parsed_args.nodes_json})
        })
