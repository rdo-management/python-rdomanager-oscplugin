...
class DeployOvercloud(command.Command):
    ...
    def _update_paramaters(self, args, network_client, stack):
        ...
        scale_controller = (parameters.get('ControllerCount', 0) > 1 or
                            parameters.get('Controller-1::count', 0) > 1)
        ...
        if args.templates:
            parameters.update({
                ...
                'NeutronDhcpAgentsPerNetwork': neutron_dhcp_agents_per_network,
            })
        else:
            parameters.update({
                'Controller-1::NeutronL3HA': neutron_l3_ha,
                'Controller-1::NeutronAllowL3AgentFailover': neutron_allow_l3_agent_failover,
                'Controller-1::NeutronDhcpAgentsPerNetwork': neutron_dhcp_agents_per_network,
        ...
        use_cephstorage = (parameters.get('CephStorageCount', 0) > 0 or
