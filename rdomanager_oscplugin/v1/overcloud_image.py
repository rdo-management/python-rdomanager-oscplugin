    ...
    def _get_image(self, name):
        try:
            ...
            if 'More than one image exists' in e.args[0]:
                print(('ERROR: Image "%s" already exists in glance more than'
                      ' once, delete all copies except the first one.' % name),
                      file=sys.stderr)
                exit(1)
            else:
            ...
            if self._image_changed(image_name, image_file):
                if parsed_args.update_existing:
                    self.app.client_manager.image.images.update(
                        image.id,
                        name='%s_before-%i' % (image.name, time.time())
                    )
                    return None
                else:
                ...
                print(('Image "%s" already exists and is up-to-date,'
                      ' skipping.' % image_name), file=sys.stdout)
                return image
        else:
            return None
        ...
        if os.path.isfile(dest_file):
            if self._files_changed(src_file, dest_file):
                ...
                print(('Image file "%s" already exists and is up-to-date,'
                      ' skipping.' % dest_file), file=sys.stdout)
        else:
            self._copy_file(src_file, dest_file)
