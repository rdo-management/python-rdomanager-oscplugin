...
class CreatePlugin(command.Command):
    ...
    def _load_image(self, image_path, image_name, image_client):
        ...
        kernel_id = image_client.images.create(
            ...
            is_public=True,
            disk_format='aki',
            data=self._read_image_file(image_path, '%s.vmlinuz' % image_name)
        ).id
        ...
        ramdisk_id = image_client.images.create(
            ...
            is_public=True,
            disk_format='ari',
            data=self._read_image_file(image_path, '%s.initrd' % image_name)
        ).id
        ...
        image_client.images.create(
            ...
            data=self._read_image_file(image_path, '%s.qcow2' % image_name)
        )
    ...
    def take_action(self, parsed_args):
        ...
        self.log.debug("check env variables")

        self._env_variable_or_set('OS_AUTH_URL', '')
        if os.environ['OS_AUTH_URL'] == '':
            print('Error: You must source a stackrc file for the Undercloud.')
        ...
        image_client.images.create(
            ...
            is_public=True,
            disk_format='aki',
            data=self._read_image_file(parsed_args.image_path, '%s.kernel' %
        ...
        image_client.images.create(
            ...
            is_public=True,
            disk_format='ari',
            data=self._read_image_file(parsed_args.image_path, '%s.initramfs' %
                                       os.environ['DEPLOY_NAME'])
        )
