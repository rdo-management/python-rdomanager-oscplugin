...
class ConfigureBaremetalBoot(command.Command):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument('--image-kernel',
                            default='bm-deploy-kernel',
                            help='Image with deploy kernel.')
        parser.add_argument('--image-ramdisk',
    ...
    def take_action(self, parsed_args):
        ...
        try:
            kernel_id = osc_utils.find_resource(
                image_client.images, parsed_args.image_kernel).id
        except AttributeError:
            self.log.error("Please make sure that image named \"%s\" exists in"
                           " glance and is only one with this name."
                           % parsed_args.image_ramdisk)
            return

        try:
            ramdisk_id = osc_utils.find_resource(
                image_client.images, parsed_args.image_ramdisk).id
        except AttributeError:
            self.log.error("Please make sure that image named \"%s\" exists in"
                           " glance and is only one with this name."
                           % parsed_args.image_ramdisk)
