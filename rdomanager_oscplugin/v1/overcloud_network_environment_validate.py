...
class ValidateOvercloudNetworkEnvironment(command.Command):
    """Validate the network-environment.yaml file."""

    auth_required = False
    log = logging.getLogger(__name__ + ".ValidateOvercloudNetworkEnvironment")

    def get_parser(self, prog_name):
        parser = super(ValidateOvercloudNetworkEnvironment, self).get_parser(prog_name)
        parser.add_argument(
            '-f', '--file', dest='netenv',
            help="Path to network-environment.yaml",
                ...
                for range in ranges:
                    if not subnet_obj.overlaps(range):
                        self.log.error(
                            ...
                            .format(poolitem, pooldata, subnet_item, subnet_obj))
                        self.error_count += 1
                        break
            ...
            for bridge in bridges:
                if bridge['type'] == 'ovs_bridge':
                    ...
                    if bond_count == 0:
                        ...
                        # catch it
                        self.log.debug(
                            'There are 0 bonds for bridge %s of resource %s in %s',
                            bridge['name'], item, path)
                    if bond_count == 1:
                        self.log.debug(
                            'There is 1 bond for bridge %s of resource %s in %s',
                            bridge['name'], item, path)
                    if bond_count == 2:
                        self.log.error(
                    ...
                    if bond_count == 0 and interface_count > 1:
                        self.log.error(
                            ...
                            'can only be 1 interface for bridge %s of resource %s in %s',
                            bridge['name'], item, path)
                        self.error_count += 1
